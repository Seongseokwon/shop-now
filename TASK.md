...
> 마지막 업데이트: 2026-06-08 (코드 분석 기반 9차 재정렬)
...
> 9차 업데이트: 실제 코드 분석 결과 P0-2/P0-3 완료, P0-4 decide 페이지만 적용 확인.
...
- [x] CTA 문구 전체 교체 — "쿠팡에서 보기" → "최저가 확인하기" (`CoupangButton.tsx`, `decide/page.tsx`)
- [x] 쿠팡 파트너스 안내 문구 추가 — 모든 결과 페이지(gift/decide/budget/home) + `JudgmentCard.tsx`
- [x] 네이버쇼핑 비교 링크 추가 — `decide/page.tsx` 직링 카드 및 대안 상품 카드 (gift/budget 미적용)
...
| ~~P0-2~~ | ~~CTA 문구 변경~~ | ~~완료~~ | — | — | Dev | ✅ 완료 (`커밋 67fb9a6`) |
| ~~P0-3~~ | ~~파트너스 안내 문구 추가~~ | ~~완료~~ | — | — | Dev | ✅ 완료 (`커밋 fdd0da6`) |
| P0-4 | **네이버쇼핑 비교 링크 추가** — gift/budget 페이지 미적용 | decide는 완료. gift·budget 결과 카드에도 "네이버에서도 검색하기" 버튼 필요. | 7/9/10 | 간접 | Dev | 🔶 부분 완료 (decide만) |
| P0-5 | **쿠팡 파트너스 대시보드 주 2회 체크 루틴** | 클릭/구매 데이터 없으면 수익 구조가 작동하는지 모른다. | 8/10/10 | — | 운영 | 미수립 |
| P0-6 | **result 페이지 fold 검증** — NudgeCTA가 스크롤 없이 보이는지 실기기 확인 | JudgmentCard 내부에 NudgeCTA 포함돼 있음. 실제 모바일 fold 기준 노출 여부 미확인. | 7/10/9 | 직결 | Dev | 미확인 |
...
| ~~CTA 문구 A/B — "쿠팡에서 보기" vs "최저가 확인하기"~~ | ~~완료 — 이미 "최저가 확인하기"로 교체됨. A/B 불필요~~ | — | 완료 |
...
- [x] **완료:** CTA 문구 "쿠팡에서 보기" → "최저가 확인하기" 전체 교체 (`커밋 67fb9a6`)
- [x] **완료:** 파트너스 안내 한 줄 — 모든 결과 페이지 + JudgmentCard (`커밋 fdd0da6`)
- [x] **완료:** decide 페이지 직링 카드·대안 카드에 네이버쇼핑 비교 링크 (`커밋 fdd0da6`)
- [ ] **당장:** 클리앙/뽐뿌에 "나 AI한테 [상품] 사지 말라고 혼났다" 형식 결과 공유글 게시
- [ ] **당장:** gift·budget 결과 카드에도 "네이버에서도 검색하기" deeplink 버튼 추가 (P0-4 잔여)
- [ ] **이번 주:** result 페이지 실기기(모바일) 확인 — NudgeCTA fold 내 노출 여부 (P0-6)
- [ ] **이번 주:** 쿠팡 파트너스 대시보드 최초 확인 후 클릭/전환 수 기록
